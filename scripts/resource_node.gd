...
func _configure_art_anchors() -> void:
	...
	for art: Sprite2D in [wood_art, stone_art, iron_art, essence_art]:
		art.centered = false
		art.scale = Vector2.ONE * visual_scale
		var alignment := ASSETS.alignment_for_texture(art.texture, visual_scale)
		art.position = alignment["sprite_position"]
...
func _configure_collection_area() -> void:
	...
	collection_shape.position = resource_profile["collection_position"]
...
func _on_body_entered(body: Node) -> void:
	...
	if main.has_method("try_collect_resource") and main.try_collect_resource(resource_type, amount):
		collected = true

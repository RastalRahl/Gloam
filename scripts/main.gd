...
func _can_skip_to_night() -> bool:
	return (
		run_started
		and current_phase == Phase.DAY
		and not dusk_return_pending
		and not game_over
		and not get_tree().paused
		and _player_can_act()
		and _is_player_in_village()
		and not village_management_open
		and not interaction_menu_open
		and not weapon_panel.visible
		and not level_up_panel.visible
		and not game_over_panel.visible
		and not victory_panel.visible
	)
...
func _refresh_skip_to_night_control() -> void:
	...
	if not _is_player_in_village():
		skip_to_night_button.tooltip_text = "Return inside the village before starting the assault."
	elif skip_to_night_button.disabled:
		skip_to_night_button.tooltip_text = "Close menus and finish the current action first."
	else:
		skip_to_night_button.tooltip_text = "End the day early after a short confirmation."
...
func _on_skip_to_night_pressed() -> bool:
	if not _can_skip_to_night():
		_disarm_skip_confirmation()
		_show_toast("Return inside the village and close menus first", "warning")
		return false
	if skip_confirmation_time_left <= 0.0:
...
func _on_night_hostile_defeated(enemy: GloamEnemy) -> void:
	if is_instance_valid(wave_director):
		wave_director.mark_required_hostile_killed(enemy)
...
func _on_grave_ox_defeated() -> void:
	...
	if is_instance_valid(wave_director) and is_instance_valid(defeated_boss):
		wave_director.mark_required_hostile_killed(defeated_boss)
	current_boss = null
	boss_panel.hide()
...
func try_rescue_villager() -> bool:
	return population_manager.try_rescue_villager() if is_instance_valid(population_manager) else false
